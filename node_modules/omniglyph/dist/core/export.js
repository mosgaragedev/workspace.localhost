/**
 * Core logic for `omniglyph export` — renders a source text to PNG pages, extracts
...
// consumers import (omniglyph/transform → renderTextToImages), not the
...
export function buildPromptText(pageCount, factsheet, files, droppedItems = 0) {
    ...
    return (`These ${pageCount} image${pageCount !== 1 ? 's' : ''} contain source code/text rendered as PNG pages by OmniGlyph.\n\n` +
        fileSection +
...
export async function runExportCore(sourceText, opts) {
    ...
    // SDK consumers (omniglyph/transform).
