...
//#region src/useSmoothStreamContent.ts
const DEFAULT_BYPASS_LANGUAGES = ["html"];
const PRESET_CONFIG = {
	balanced: {
...
const useSmoothStreamContent = (content, { enabled = true, preset = "balanced", tailUnitsRef } = {}) => {
	const config = PRESET_CONFIG[preset];
	const profiler = useStreamdownProfiler();
	const [displayedContent, setDisplayedContent] = useState(content);
	const [lexBufferedContent] = useState(createBlockLexer);
	const completePrefixRef = useRef({
		content: "",
		count: 0
	});
	...
	const startFrameLoop = useCallback(() => {
		...
		const tick = (ts) => {
			...
			const commitIntervalMs = Math.min(MAX_COMMIT_INTERVAL_MS, config.minCommitIntervalMs * (1 + tailUnits / COMMIT_INTERVAL_TAIL_SCALE_UNITS));
			...
			if (inputActive) {
				...
				const idleFlushCps = Math.max(config.flushCps, baseCps * 1.8, arrivalCpsEmaRef.current * .8);
			...
			let revealChars = Math.max(inputActive ? urgentBacklog || burstyInput ? 2 : 1 : 2, Math.round(currentCps * dtSeconds));
			if (inputActive) {
			...
			if (completePrefixRef.current.content !== targetContentRef.current) {
				...
				const completeEnd = backlog > revealChars && /\n[\t \r]*\n/.test(pending) ? lexBufferedContent(target).completeEnd : 0;
				completePrefixRef.current = {
					content: target,
					count: displayedCount + countChars(target.slice(displayedContentRef.current.length, completeEnd))
				};
			}
