 ...
 */
async function getCommandVersion(versionCommand, parser) {
    return new Promise((resolve) => {
...
function compareVersions(current, required) {
    ...
    for (let i = 0; i < Math.max(parseCurrent.length, parseRequired.length); i++) {
...
export async function checkDependencies(dependencies) {
    ...
    for (const dep of dependencies) {
        ...
        if (dep.minVersion && dep.versionCommand) {
            const currentVersion = await getCommandVersion(dep.versionCommand, dep.versionParser);
            if (currentVersion) {
 ...
 */
export function formatDependencyError(toolName, missingDeps, versionMismatches) {
