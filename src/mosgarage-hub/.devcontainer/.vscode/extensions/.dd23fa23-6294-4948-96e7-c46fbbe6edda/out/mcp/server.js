...
import { createZenOfAnsibleHandler, createListToolsHandler, createAnsibleLintHandler, createAnsibleNavigatorHandler, createADEEnvironmentInfoHandler, createADESetupEnvironmentHandler, createADTCheckEnvHandler, createDefineAndBuildExecutionEnvHandler, createAgentsGuidelinesHandler, } from "./handlers.js";
import { checkDependencies, formatDependencyError, } from "./dependencyChecker.js";
import { createProjectsHandler } from "./tools/creator.js";
import { getEERules, getExecutionEnvironmentSchema, getSampleExecutionEnvironment, } from "./resources/eeSchema.js";
import { getAgentsGuidelines } from "./resources/agents.js";
export function createAnsibleMcpServer(workspaceRoot) {
    const server = new McpServer({
        name: "ansible-mcp-server",
        version: "0.1.0",
    });
    // Register execution environment schema as a resource
    server.registerResource("execution-environment-schema", "schema://execution-environment", {
        title: "Execution Environment Schema",
        description: "JSON schema for Ansible execution environment definition files. " +
    ...
    server.registerResource("execution-environment-sample", "sample://execution-environment", {
        title: "Sample Execution Environment File",
        description: "A sample execution-environment.yml file that demonstrates the v3 schema structure. " +
    ...
    server.registerResource("execution-environment-rules", "rules://execution-environment", {
        title: "Execution Environment Rules",
        description: "The ee-rules.md file contains rules and guidelines for Ansible execution environment files. " +
            "Use this as a reference along with the schema to understand how to structure execution environment files. " +
    ...
    server.registerResource("ansible-content-best-practices", "guidelines://ansible-content-best-practices", {
        title: "Ansible Content Best Practices",
        description: "Comprehensive best practices and guidelines for writing Ansible content. " +
    ...
    // Register core tools
    registerToolWithDeps("zen_of_ansible", {
        title: "The Zen of Ansible",
        description: "20 aphorisms that describe Ansible's design philosophy.",
    }, createZenOfAnsibleHandler(), []);
    registerToolWithDeps("ansible_content_best_practices", {
        title: "Ansible Content Best Practices",
        description: "Get best practices and guidelines for writing Ansible content. " +
    ...
    registerToolWithDeps("list_available_tools", {
        title: "List Available Tools",
        description: "Shows all available Ansible MCP tools. Use this to discover what tools you can use. " +
    ...
    registerToolWithDeps("ansible_lint", {
        title: "Ansible Lint",
        description: "**PREFERRED APPROACH**: Instead of using this MCP tool, use terminal commands to run ansible-lint. " +
        ...
        inputSchema: {
            filePath: z
                .string()
                .describe("Path to the Ansible playbook file to lint. " +
    ...
    const navigatorInputSchema = {
        userMessage: z
            .string()
            .describe("**REQUIRED:** The user's original message/prompt. Just pass it as-is (DO NOT leave empty). " +
        ...
        filePath: z
            .string()
            .optional()
            .describe("Advanced: Direct file path to the playbook. " +
        ...
        mode: z
            ...
            .optional()
            .default("stdout")
            .describe("Output mode: 'stdout' (default for this tool, direct terminal output) or 'interactive' (TUI for exploration). " +
            "Only specify if user explicitly requests a different mode."),
        environment: z
            .string()
            .optional()
            .default("auto")
            .describe("Environment selection: 'auto' (default, checks PATH first, then venv), 'system' (only use PATH/system), " +
        ...
        disableExecutionEnvironment: z
            ...
            .optional()
            .default(false)
            .describe("Set to `true` to disable execution environments (passes --ee false). " +
    ...
    registerToolWithDeps("ansible_navigator", {
        title: "Ansible Navigator",
        description: "Runs Ansible playbooks using ansible-navigator with smart features and provides information about available modes and options. " +
            "\n\n" +
    ...
    registerToolWithDeps("ade_environment_info", {
        title: "ADE Environment Information",
        description: "Get comprehensive environment information including Python, Ansible, ADE, ADT status, and installed collections. " +
    ...
    registerToolWithDeps("ade_setup_environment", {
        title: "ADE Setup Development Environment",
        description: "Set up a complete Ansible development environment using ADE. Creates virtual environments, installs collections, and manages dependencies. " +
    ...
    registerToolWithDeps("adt_check_env", {
        title: "ADT Check and Install Development Tools",
        description: "Check if ADT (ansible-dev-tools) is installed and install it if missing. " +
    ...
    registerToolWithDeps("create_ansible_projects", {
        title: "Create Ansible Projects",
        description: "Create new Ansible collection or playbook project using ansible-creator. " +
        ...
        inputSchema: {
            projectType: z
                ...
                .optional()
                .describe("The type of project to create: 'collection' for Ansible collections or 'playbook' for playbook projects. " +
                "If not provided, the tool will prompt you to specify this first."),
            namespace: z
                .string()
                .optional()
                .describe("The collection namespace (e.g., 'test_org'). " +
                "Required for both collection and playbook projects. " +
                "Must be provided along with 'collectionName'."),
            collectionName: z
                .string()
                .optional()
                .describe("The collection name (e.g., 'test_coll'). " +
                "Required for both collection and playbook projects. " +
            ...
            projectDirectory: z
                .string()
                .optional()
                .describe("The name of the project directory where the project will be created (e.g., 'my_playbook_proj', 'my_collection_proj'). " +
                "The directory will be created inside the workspace. " +
                "If not provided, the tool will prompt you to specify this."),
            path: z
                .string()
                .optional()
                .describe("Advanced: Full destination directory path for the project. " +
    ...
    registerToolWithDeps("define_and_build_execution_env", {
        title: "Define and Build Execution Environment",
        description: "Create an EE definition file for building Ansible execution environment images with ansible-builder. " +
        ...
        inputSchema: {
            baseImage: z
                .string()
                .describe("The base container image to use (e.g., 'quay.io/fedora/fedora-minimal:41' or 'quay.io/centos/centos:stream10')"),
            tag: z
                .string()
                .describe("The tag/name for the resulting execution environment image (e.g., 'my-ee:latest')"),
            destinationPath: z
                .string()
                .optional()
                .describe("Optional destination directory path for the execution-environment.yml file. Defaults to workspace root."),
            collections: z
                .array(z.string())
                .optional()
                .describe("Optional array of Ansible collection names to include (e.g., ['amazon.aws', 'ansible.utils'])"),
            systemPackages: z
                .array(z.string())
                .optional()
                .describe("Optional array of system packages to install (e.g., ['git', 'vim', 'curl'])"),
            pythonPackages: z
                .array(z.string())
                .optional()
                .describe("Optional array of Python packages to install (e.g., ['boto3', 'requests'])"),
            generatedYaml: z
                .string()
                .optional()
                .describe("Internal parameter: LLM-generated YAML content. Use this when calling the tool a second time after generating the YAML from the prompt provided in the first call."),
        },
        annotations: {
            keywords: [
    ...
    async (request) => {
        ...
        if (dependencies.length > 0) {
            ...
            if (!depCheck.satisfied) {
                const errorMessage = formatDependencyError(toolName, depCheck.missingDependencies, depCheck.versionMismatches);
        ...
        const availableToolNames = handlers
            ...
            : "no handlers object";
        console.error(`[MCP] Tool '${toolName}' handler not found. Available handlers: ${availableToolNames}`);
        throw new McpError(ErrorCode.InternalError, `Tool '${toolName}' handler not found. Available tools in registry: ${availableToolNames}`);
    });
    return server;
}
export async function runStdio(_workspaceRoot) {
