...
import { runAnsibleNavigator, formatNavigatorResult, } from "./tools/ansibleNavigator.js";
import { getEnvironmentInfo, setupDevelopmentEnvironment, checkAndInstallADT, formatEnvironmentInfo, } from "./tools/adeTools.js";
import { generateExecutionEnvironment, formatExecutionEnvResult, buildEEStructureFromPrompt, } from "./tools/executionEnv.js";
...
export function createAnsibleLintHandler() {
    return async (args) => {
        try {
            ...
            const { result: lintingResult, fixedContent } = await runAnsibleLint(args.filePath, fix);
            // Ensure the result is an array before formatting
            const resultArray = Array.isArray(lintingResult) ? lintingResult : [];
            const formattedResult = formatLintingResult(resultArray, fix, fixedContent, args.filePath);
            return {
...
export function createWorkspaceFileHandler(workspaceRoot) {
...
export function createListToolsHandler(getToolNames) {
    return async () => {
...
export function createADEEnvironmentInfoHandler(workspaceRoot) {
    return async () => {
        try {
...
export function createADESetupEnvironmentHandler(workspaceRoot) {
    return async (args) => {
        try {
...
export function createAnsibleNavigatorHandler() {
    return async (args, workspaceRoot) => {
        ...
        if (!targetFilePath && args.userMessage && workspaceRoot) {
            ...
            const explicitPathMatch = args.userMessage.match(/(?:playbooks\/)?[\w-]+\.(?:yml|yaml)/);
            if (explicitPathMatch) {
                ...
                if (!targetFilePath.startsWith("playbooks/") &&
        ...
        if (!targetFilePath) {
            return {
                content: [
                    {
                        type: "text",
                        text: "❌ **Could not determine which playbook to run.**\n\n" +
        ...
        const disableExecutionEnvironment = args.disableExecutionEnvironment || false;
        ...
        try {
            const { output, debugOutput, navigatorPath, executionEnvironmentDisabled, } = await runAnsibleNavigator(normalizedFilePath, mode, workspaceRoot, disableExecutionEnvironment, environment);
            const formattedResult = formatNavigatorResult(output, debugOutput, normalizedFilePath, mode, executionEnvironmentDisabled ?? disableExecutionEnvironment, navigatorPath, environment);
            return {
                content: [{ type: "text", text: formattedResult }],
            };
        }
            ...
            const isContainerEngineError = errorMessageLower.includes("container engine") ||
            ...
            if (isContainerEngineError && !disableExecutionEnvironment) {
                ...
                try {
                    ...
                    const { output, debugOutput, navigatorPath, executionEnvironmentDisabled, } = await runAnsibleNavigator(normalizedFilePath, mode, workspaceRoot, true, // Force disable execution environment
                    environment);
                    const formattedResult = formatNavigatorResult(output, debugOutput, normalizedFilePath, mode, executionEnvironmentDisabled ?? true, navigatorPath, environment);
                    return {
                        content: [
                            {
                                type: "text",
...
export function createDefineAndBuildExecutionEnvHandler(workspaceRoot) {
    return async (args) => {
        ...
        try {
            ...
            if (!args.baseImage || !args.tag) {
                console.log("[EE Tool Handler] Validation failed: missing baseImage or tag");
                return {
                    content: [
                        {
                            type: "text",
            ...
            if (!args.generatedYaml) {
                console.log("[EE Tool Handler] No generatedYaml provided - returning prompt for LLM to generate YAML");
                const { prompt } = await buildEEStructureFromPrompt(args);
                console.log("[EE Tool Handler] Prompt generated, length:", prompt.length);
                return {
                    content: [
                        {
                            type: "text",
            ...
            console.log("[EE Tool Handler] generatedYaml IS provided - proceeding to generate EE file");
            console.log("[EE Tool Handler] Generating EE file with LLM-generated YAML");
            console.log("[EE Tool Handler] generatedYaml length:", args.generatedYaml?.length);
            console.log("[EE Tool Handler] generatedYaml preview (first 200 chars):", args.generatedYaml?.substring(0, 200));
            const result = await generateExecutionEnvironment(args, workspaceRoot, args.generatedYaml);
            ...
            console.log("[EE Tool] Formatted output preview (first 500 chars):", formattedOutput.substring(0, 500));
            const response = {
                content: [
                    {
                        type: "text",
            ...
            console.log("[EE Tool] Returning response with content length:", response.content[0].text.length);
            console.log("[EE Tool] Response content preview (first 500 chars):", response.content[0].text.substring(0, 500));
            return response;
        }
        catch (error) {
