...
describe('ProjectsController', function () {
    ...
    describe('project controller operations', function () {
        describe('Project file operations and prompting', function () {
            it('Should create new sqlproj file with correct specified target platform', async function () {
            ...
            it('Should create new edge project with expected template files', async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const projFileDir = await testUtils.generateTestFolderPath(this.test);
                const projFilePath = await projController.createNewProject({
                    newProjName: 'TestProjectName',
            ...
            it('Should return silently when no SQL object name provided in prompts', async function () {
                for (const name of ['', '    ', undefined]) {
                    ...
                    await projController.addItemPrompt(new project_1.Project('FakePath'), '', { itemType: "script" /* ItemType.script */ });
            ...
            it('Should show error if trying to add a file that already exists', async function () {
            ...
            it('Should not create file if no itemTypeName is selected', async function () {
            ...
            it('Should add existing item', async function () {
                const tableName = 'table1';
                sinon.stub(vscode.window, 'showInputBox').resolves(tableName);
                sinon.stub(utils, 'sanitizeStringForFilename').returns(tableName);
                const spy = sinon.spy(vscode.window, 'showErrorMessage');
            ...
            it('Should show error if trying to add a folder that already exists', async function () {
            ...
            it('Should be able to add folder with reserved name as long as not at project root', async function () {
                const folderName = 'folder1';
            ...
            it('Should create .vscode/tasks.json with isDefault=true when configureDefaultBuild is true', async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const projFileDir = await testUtils.generateTestFolderPath(this.test);
            ...
            it.skip('Should delete nested ProjectEntry from node', async function () {
            ...
            it('Should delete database references', async function () {
                ...
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                sinon.stub(vscode.window, 'showWarningMessage').returns(Promise.resolve(constants.yesString));
            ...
            it('Should exclude nested ProjectEntry from node', async function () {
            ...
            it('Should exclude a folder', async function () {
                let proj = await testUtils.createTestSqlProject(this.test);
            ...
            // TODO: move test to DacFx and fix delete
            it.skip('Should delete folders with excluded items', async function () {
                let proj = await testUtils.createTestProject(this.test, templates.newSqlProjectTemplate);
                const setupResult = await setupDeleteExcludeTest(proj);
            ...
            it('Should reload correctly after changing sqlproj file', async function () {
                // create project
            ...
            it('Should be able to add pre deploy and post deploy script', async function () {
                ...
                await projController.addItemPrompt(project, '', { itemType: "preDeployScript" /* ItemType.preDeployScript */ });
                ...
                await projController.addItemPrompt(project, '', { itemType: "postDeployScript" /* ItemType.postDeployScript */ });
                should(project.postDeployScripts.length).equal(1, 'Post deploy script should be successfully added');
            });
            it('Should be able to add publish profile', async function () {
                ...
                await projController.addItemPrompt(project, '', { itemType: "publishProfile" /* ItemType.publishProfile */ });
                should(project.publishProfiles.length).equal(1, 'Publish profile should be successfully added.');
            });
            it('Should change target platform', async function () {
                sinon.stub(vscode.window, 'showQuickPick').resolves({ label: "Azure SQL Database" /* SqlTargetPlatform.sqlAzure */ });
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        describe('Publishing and script generation', function () {
            it('Publish dialog should open from ProjectController', async function () {
                let opened = false;
                let publishDialog = TypeMoq.Mock.ofType(publishDatabaseDialog_1.PublishDatabaseDialog);
                publishDialog.setup(x => x.openDialog()).returns(() => { opened = true; });
                let projController = TypeMoq.Mock.ofType(projectController_1.ProjectsController);
                projController.callBase = true;
                projController.setup(x => x.getPublishDialog(TypeMoq.It.isAny())).returns(() => publishDialog.object);
                const proj = new project_1.Project('FakePath');
            ...
            it('Callbacks are hooked up and called from Publish dialog', async function () {
                ...
                const proj = await project_1.Project.openProject(projectFile);
                ...
                const setupPublishDialog = () => {
                    const dialog = new publishDatabaseDialog_1.PublishDatabaseDialog(proj);
            ...
            it('Should copy dacpac to temp folder before publishing', async function () {
                ...
                testContext.dacFxService.setup(x => x.generateDeployScript(TypeMoq.It.isAny(), TypeMoq.It.isAny(), TypeMoq.It.isAny(), TypeMoq.It.isAny(), TypeMoq.It.isAny(), TypeMoq.It.isAny())).returns(async (p) => {
                    ...
                    postCopyContents = (await fs_1.promises.readFile(publishedDacpacPath)).toString();
                    return Promise.resolve(testContext_1.mockDacFxResult);
                });
                let projController = TypeMoq.Mock.ofType(projectController_1.ProjectsController);
                projController.callBase = true;
                ...
                await fs_1.promises.rm(publishedDacpacPath);
            });
        });
    });
    describe('Create project from database operations and dialog', function () {
        afterEach(() => {
            sinon.restore();
        });
        it('Should create list of all files and folders correctly', async function () {
        ...
        it('Should error out for inaccessible path', async function () {
            const spy = sinon.spy(vscode.window, 'showErrorMessage');
        ...
        it('Create project from Database dialog should open from ProjectController', async function () {
            let opened = false;
            let createProjectFromDatabaseDialog = TypeMoq.Mock.ofType(createProjectFromDatabaseDialog_1.CreateProjectFromDatabaseDialog, undefined, undefined, testContext_1.mockConnectionProfile);
            ...
            await projController.object.createProjectFromDatabase(testContext_1.mockConnectionProfile);
            should(opened).equal(true);
        });
        it.skip('Callbacks are hooked up and called from create project from database dialog', async function () {
            const createProjectFromDbHoller = 'hello from callback for createProjectFromDatabase()';
            let holler = 'nothing';
            const createProjectFromDatabaseDialog = TypeMoq.Mock.ofType(createProjectFromDatabaseDialog_1.CreateProjectFromDatabaseDialog, undefined, undefined, undefined);
        ...
        it('Should set model filePath correctly for ExtractType = File', async function () {
            let folderPath = await testUtils.generateTestFolderPath(this.test);
            let projectName = 'My Project';
            let importPath;
            let model = { connectionUri: 'My Id', database: 'My Database', projName: projectName, filePath: folderPath, version: '1.0.0.0', extractTarget: 1 /* mssql.ExtractTarget['file'] */, sdkStyle: false };
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        it('Should set model filePath correctly for ExtractType = Schema/Object Type', async function () {
            let folderPath = await testUtils.generateTestFolderPath(this.test);
            let projectName = 'My Project';
            let importPath;
            let model = { connectionUri: 'My Id', database: 'My Database', projName: projectName, filePath: folderPath, version: '1.0.0.0', extractTarget: 5 /* mssql.ExtractTarget['schemaObjectType'] */, sdkStyle: false };
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            projController.setFilePath(model);
            importPath = model.filePath;
    ...
    describe('Add database reference', function () {
        it('Add database reference dialog should open from ProjectController', async function () {
            let opened = false;
            let addDbReferenceDialog = TypeMoq.Mock.ofType(addDatabaseReferenceDialog_1.AddDatabaseReferenceDialog);
            ...
            await projController.object.addDatabaseReference(new project_1.Project('FakePath'));
            should(opened).equal(true);
        });
        it('Callbacks are hooked up and called from Add database reference dialog', async function () {
            const projPath = path.dirname(await testUtils.createTestSqlProjFile(this.test, baselines.openProjectFileBaseline));
            const proj = new project_1.Project(projPath);
            const addDbRefHoller = 'hello from callback for addDatabaseReference()';
            let holler = 'nothing';
            const addDbReferenceDialog = TypeMoq.Mock.ofType(addDatabaseReferenceDialog_1.AddDatabaseReferenceDialog, undefined, undefined, proj);
            addDbReferenceDialog.callBase = true;
            addDbReferenceDialog.setup(x => x.addReferenceClick()).returns(() => {
                return projController.object.addDatabaseReferenceCallback(proj, { systemDb: 0 /* SystemDatabase.Master */, databaseName: 'master', suppressMissingDependenciesErrors: false, systemDbReferenceType: 0 /* mssql.SystemDbReferenceType.ArtifactReference */ }, { treeDataProvider: new databaseProjectTreeViewProvider_1.SqlDatabaseProjectTreeViewProvider(), element: undefined });
            });
            addDbReferenceDialog.setup(x => x.openDialog()).returns(() => Promise.resolve());
            const projController = TypeMoq.Mock.ofType(projectController_1.ProjectsController);
            projController.callBase = true;
            projController.setup(x => x.getAddDatabaseReferenceDialog(TypeMoq.It.isAny())).returns(() => addDbReferenceDialog.object);
            projController.setup(x => x.addDatabaseReferenceCallback(TypeMoq.It.isAny(), TypeMoq.It.is((_) => true), TypeMoq.It.isAny())).returns(() => {
        ...
        it.skip('Should not allow adding circular project references', async function () {
        ...
        it.skip('Should add dacpac references as relative paths', async function () {
    ...
    describe('AutoRest generation', function () {
        // skipping for now because this feature is hidden under preview flag
        it.skip('Should create project from autorest-generated files', async function () {
    ...
    describe('Move file', function () {
        it('Should move a file to another folder', async function () {
            const spy = sinon.spy(vscode.window, 'showErrorMessage');
            sinon.stub(vscode.window, 'showWarningMessage').returns(Promise.resolve(constants.move));
        ...
        it('Should move a folder to another folder', async function () {
            const spy = sinon.spy(vscode.window, 'showErrorMessage');
            sinon.stub(vscode.window, 'showWarningMessage').returns(Promise.resolve(constants.move));
        ...
        it('Should not allow moving a file to Database References or SQLCMD folder', async function () {
            const spy = sinon.spy(vscode.window, 'showErrorMessage');
            sinon.stub(vscode.window, 'showWarningMessage').returns(Promise.resolve(constants.move));
            let proj = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const projTreeRoot = await setupMoveTest(proj);
        ...
        it('Should only allow moving files and folders', async function () {
            const spy = sinon.spy(vscode.window, 'showErrorMessage');
            let proj = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const projTreeRoot = await setupMoveTest(proj);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        it('Should not allow moving files between projects', async function () {
            const spy = sinon.spy(vscode.window, 'showErrorMessage');
            sinon.stub(vscode.window, 'showWarningMessage').returns(Promise.resolve(constants.move));
    ...
    describe('Rename file', function () {
        it('Should not do anything if no new name is provided', async function () {
        ...
        it('Should rename a sql object file', async function () {
        ...
        it('Should rename a pre and post deploy script', async function () {
            let proj = await testUtils.createTestProject(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
    ...
    describe('SqlCmd Variables', function () {
        it('Should delete sqlcmd variable', async function () {
        ...
        it('Should add sqlcmd variable', async function () {
            let project = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const sqlProjectsService = await utils.getSqlProjectsService();
            await sqlProjectsService.openProject(project.projectFilePath);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        it('Should add sqlcmd variable without DefaultValue', async function () {
            let project = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const sqlProjectsService = await utils.getSqlProjectsService();
            await sqlProjectsService.openProject(project.projectFilePath);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        it('Should update sqlcmd variable', async function () {
            let project = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const sqlProjectsService = await utils.getSqlProjectsService();
            await sqlProjectsService.openProject(project.projectFilePath);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        it('Should remove file extensions from user input when creating files', async function () {
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            let project = await testUtils.createTestProject(this.test, baselines.newProjectFileBaseline);
