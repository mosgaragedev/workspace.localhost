...
class WebViewPanels {
    ...
    getHtmlWebviewForSamplesView(searchQuery = '') {
        if (this.sampleView === null) {
            this.sampleView = vscode.window.createWebviewPanel('CLISamples', 'CLI for Microsoft 365 - samples', vscode.ViewColumn.One, {
                enableScripts: true,
                localResourceRoots: [this.context.extensionUri],
    ...
    getHtmlWebviewForDocsView(commandName) {
        if (this.docsView === null) {
            this.docsView = vscode.window.createWebviewPanel('CLIManual', 'CLI for Microsoft 365 - docs', vscode.ViewColumn.One, {
                enableScripts: true,
                localResourceRoots: [this.context.extensionUri],
    ...
    _createScriptFile(sampleTitle, type) {
        ...
        axios_1.default
            ...
            if (sample.source === 'cli') {
                ...
                const index = sample.type.indexOf(type);
